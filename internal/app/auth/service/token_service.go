...
func (t *tokenService) Create(user entity.User, ip string, device string) (token string, deviceId string, e error) {
	...
	dId := unique.Key(52)
	// store refresh token
	_, err = repository.NewUserToken().Create(context.Background(), entity.UserToken{
...
func (t *tokenService) Refresh(deviceId string) (token string, devId string, e error) {
	...
	userToken, errToken := userTokenRepository.FindByDeviceId(context.Background(), deviceId)
	if errToken != nil {
		return "", "", errToken
	}
	...
	_, err = userTokenRepository.Update(context.Background(), userToken.ID, userToken)
	if err != nil {
		return "", "", err
	}
